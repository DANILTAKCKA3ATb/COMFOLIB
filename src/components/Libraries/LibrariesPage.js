...
import { Backdrop, Button, CircularProgress, Divider, IconButton, Paper, Stack, TextField } from '@mui/material';
...
const LibrariesPage = ({ isAdminPage, ...props }) => {
    ...
    const { libraries, searchLibraryInProgress, searchLibraryError, searchText } = useSelector(state => state.searchLibrary);
    ...
    return (
        ...
        >
            <Backdrop sx={{ color: '#fff', zIndex: theme => theme.zIndex.drawer + 1 }} open={searchLibraryInProgress || deleteLibraryInProgress}>
            ...
            <Stack sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center' }}>
                ...
                {libraries.map(library => {
                    if (textFilter(library)) return <LibraryCard library={library} key={library.id} isAdminPage={isAdminPage} />;
                })}
            </Stack>
        </Paper>
    );
