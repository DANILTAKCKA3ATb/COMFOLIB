...
import { setBookCover, setBookCoverFileName, setBookCoverPreview, setBookPDF, setBookPDFFileName, setBookTopics } from '../../redux/createBookReducer';
import { getTopics } from '../../redux/searchBookReducer';
//import { maxLengthCreator, minLengthCreator } from '../../utils/validators';

// const minLenght2 = minLengthCreator(2);
// const minLenght10 = minLengthCreator(10);
// const maxLenght50 = maxLengthCreator(50);
// const maxLenght500 = maxLengthCreator(500);
                        ...
                        <Stack direction='row'>
                            ...
                            <Container>
                                <Stack direction='column' gap={1}>
                                    ...
                                    <FieldTextInput required variant='outlined' type='text' id='AuthorName' label='Author' name='AuthorName' />
                                    <FieldTextInput required variant='outlined' type='number' id='Year' label='Year of writing' name='Year' />
                                    <Button variant='contained' component='label'>
                                        <Typography variant='h6'>Add book cover</Typography>
                                        {!!bookForChange ? (
                                            <input type='file' id='BookCover' name='BookCover' accept='image/*' onChange={onSelectImage} hidden />
                                        ) : (
                                            <input required type='file' id='BookCover' name='BookCover' accept='image/*' onChange={onSelectImage} hidden />
                                        )}
                                    </Button>
                                    ...
                                    <Button variant='contained' component='label'>
                                        ...
                                        {!!bookForChange ? (
                                            <input type='file' id='BookPDF' name='BookPDF' accept='.pdf' onChange={onSelectedPDFFile} hidden />
                                        ) : (
                                            <input required type='file' id='BookPDF' name='BookPDF' accept='.pdf' onChange={onSelectedPDFFile} hidden />
                                        )}
                                    </Button>
                                    <Typography variant='caption' color='green' textAlign={'center'}>
                                        {pdfFileName}
