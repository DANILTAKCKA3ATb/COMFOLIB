...
import { Autocomplete, Backdrop, Button, CircularProgress, Divider, Paper, Stack, TextField } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import ReportProblemIcon from '@mui/icons-material/ReportProblem';
import AddIcon from '@mui/icons-material/Add';
import BookCard from '../BookCard/BookCard';
import { getTopics, searchBooks, setSearchText, setSearchTopic } from '../../redux/searchBookReducer';
import { setBookForChange } from '../../redux/createBookReducer';

const BooksPage = ({ isAdminPage, ...props }) => {
    const { books, searchBookInProgress, searchBookError, searchText, searchTopic, allTopics } = useSelector(state => state.searchBook);
    const { creationBookInProgress, deleteBookInProgress, deleteBookError } = useSelector(state => state.createBook);
    ...
    return (
        ...
        >
            ...
            <Stack sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center' }}>
                ...
                {books.map(book => {
                    if (textFilter(book) && topicFilter(book)) return <BookCard book={book} key={book.id} isAdminPage={isAdminPage} />;
                })}
            </Stack>
        </Paper>
    );
